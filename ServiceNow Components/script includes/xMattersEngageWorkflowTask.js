...
gs.include("xMattersEngageWorkflowEvent");
gs.include("xMattersMomentjs");
gs.include("xMattersMomentTZ");
...
xMattersEngageWorkflowTask.prototype = {
  ...
  addTask: function(rec, triggerRule) {
    try {
      var xmEvent = new xMattersEngageWorkflowEvent(this.config, this.dataHelper, this.log.getLogTraceId());

      // Add Incident Properties
      var parentRecord = new GlideRecord(rec.parent_table_name);
      parentRecord.get(rec.parent_sys_id);
      xmEvent.addAllProperties(parentRecord, rec.parent_table_name);
      ...
      // parent info
      xmEvent.addProperty('parent_table_name', rec.parent_table_name);
      xmEvent.addProperty('parent_sys_id', rec.parent_sys_id);

      // engage workflow custom fields
      xmEvent.addProperty('task_type', rec.type);
      xmEvent.addProperty('task_date_time', rec.date_time);

      // format date time in try/catch
      var task_date_time_moment_format;
      try {
        task_date_time_moment_format = moment(rec.date_time).tz(this.engage_workflow_config.ENGAGE_WORKFLOW_MOMENT_TZ).format(this.engage_workflow_config.ENGAGE_WORKFLOW_MOMENT_FORMAT);

      } catch (e) {
        task_date_time_moment_format = rec.date_time;
      }

      xmEvent.addProperty('task_date_time_moment_format', task_date_time_moment_format);
